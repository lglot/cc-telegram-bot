...
def handle(msg: dict, state: dict) -> None:
    ...
    if text.startswith("/handoff"):
        ...
        chat_type = (msg.get("chat") or {}).get("type")
        if chat_type != "private" and not is_forum_msg(msg):
            # niente Topics (gruppo non-forum): handoff in place (come /compact ma con doc ricco)
        ...
        if tid is None:
            send(chat_id, f"❌ creazione topic fallita: {err}\nℹ️ chat privata: topics via BotFather; gruppo: bot admin con 'Manage Topics'.", thread_id=thread_id)
            return
        nskey = f"{chat_id}:{tid}"
